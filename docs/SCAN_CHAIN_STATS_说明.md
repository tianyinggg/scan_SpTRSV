...
- 流式读取并抽取严格下三角依赖：`read_strict_lower_dependencies_mtx`
- 扁平数组统计：`pred_count`、`first_pred`，不再构造完整 SciPy 矩阵或 `pred = list[list[int]]`
...
- `strict_child_count[curr]`
- `unique_strict_child[curr]`，仅当严格子节点唯一时有效
